...
- **Deploy functions** — ⚠️ **SIEMPRE con selector explícito, SIEMPRE con `--project`, NUNCA con `--force`**:
  ...
  - **Orden**: rules → functions (consumidores primero, schedulers al final) → hosting. El predeploy genera `apps/functions/.deploy`; verificar "Successful update/create operation" por función. **No inspeccionar `.deploy` con el shell parado adentro**: deja el directorio bloqueado en Windows y el predeploy siguiente falla con `EBUSY`.
  - 🚨 **`firebase deploy` devuelve EXIT 0 con funciones SIN APLICAR. VERIFICADO EN PRODUCCIÓN, DOS VECES, EN EL MISMO DEPLOY (2026-08-21, DEPLOY-TRAMO-1-BACKEND-1).** Los 429 de cuota reintentan solos **hasta agotar sus reintentos**, y cuando se agotan el CLI imprime el error de esa función y **igual cierra el comando con éxito global**. La primera vez dejó sin aplicar `onWebhookInbox` y `conversationSendAttachment`; la segunda, `metaWebhook`. Ninguna de las dos se notó por el código de salida.
    **REGLA: el éxito de un deploy se mide contrastando el `updateTime` de cada función contra producción (GCF v2, read-only), JAMÁS contra el exit code del comando.** Si se confía en el exit code, se avanza al productor con el consumidor viejo — exactamente el orden que el plan prohíbe.
    `metaWebhook` necesitó **cuatro intentos** (dos rechazados por cuota, uno perdido por un error de ruta: el `cd` no persiste dentro de un comando lanzado en background — poner el `cd` DENTRO del subshell). **No hubo downtime**: mientras se reintenta, la versión vieja de la función sigue sirviendo; el reintento es del mismo comando, no un rollback.
  - ⚠️ **El predeploy NO compila `shared`** (defecto abierto, verificado 2026-08-21). El hook corre `pnpm --filter @vpw/shared build`, que imprime **"No projects matched the filters"** y sale en 0: el filtro por nombre del paquete no matchea. El que funciona es `--filter shared`. En el deploy del 2026-08-21 no rompió nada porque `shared/dist` ya estaba compilado de las corridas de la sesión, pero **en una máquina limpia ese hook no hace nada**: correr `pnpm -r build` a mano antes de desplegar. Arreglar el filtro es otro programa.
- **Deploy hosting**: crear TEMPORAL `apps/web/.env.production.local` → `firebase deploy --only hosting --project vpw-prod-dd6ff` → **BORRAR el env temporal**.
  ...
  - **Claves obligatorias (14 — la lista de 9 de versiones previas de este runbook estaba INCOMPLETA y dejaba el panel sin conexión de Meta)**: `NEXT_PUBLIC_FIREBASE_API_KEY`, **`NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN`** (faltaba en versiones aún más viejas), `NEXT_PUBLIC_FIREBASE_PROJECT_ID`, `NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET`, `NEXT_PUBLIC_FIREBASE_MESSAGING_SENDER_ID`, `NEXT_PUBLIC_FIREBASE_APP_ID`, `NEXT_PUBLIC_USE_EMULATORS=false`, **`NEXT_PUBLIC_META_APP_ID`** y **`NEXT_PUBLIC_META_CONFIG_ID`** (si falta cualquiera de las dos, `readConfig` devuelve null — `apps/web/src/lib/metaEmbeddedSignup.ts:129-135` — e Integraciones queda en «Meta no configurada» SIN intentar el OAuth real: ninguna empresa puede conectar su número), **`NEXT_PUBLIC_META_COEXISTENCE_CONFIG_ID`** (Coexistence JAMÁS cae al config estándar — :127 —; sin esta clave la opción de conectar el número que ya usa WhatsApp Business App queda deshabilitada), `NEXT_PUBLIC_META_GRAPH_VERSION` (default `v19.0` si falta — la única con default seguro), **`NEXT_PUBLIC_SUPPORT_WHATSAPP`** (sin ella desaparecen los botones de «contactar soporte» de activación manual y RegistrationGate), **`NEXT_PUBLIC_ALLOW_SELF_REGISTRATION=false`** (si se olvida, /register vuelve a mostrar el form) y `NEXT_PUBLIC_API_BASE_URL`. Los `META_*_CONFIG_ID` y el App ID salen del App Dashboard de Meta (Embedded Signup → configuraciones) y NO están en el repo; el número de soporte lo define el owner. Plantilla completa: `apps/web/.env.production.example`.
  - **Smoke post-deploy AMPLIADO (el de chunks anterior NO detectaba este defecto)**: además de confirmar `vpw-prod-dd6ff.firebaseapp.com` presente y cero `localhost:`/`vpw-staging`/`vpw-dev`/projectId demo, verificar en los chunks descargados la PRESENCIA de los valores reales configurados de `NEXT_PUBLIC_META_APP_ID`, `NEXT_PUBLIC_META_CONFIG_ID` y `NEXT_PUBLIC_META_COEXISTENCE_CONFIG_ID` (grep del valor que se puso en el env temporal — son públicos por diseño, viajan al browser). Si alguno no aparece, el build se horneó sin esa clave y el panel quedó sin conexión de Meta aunque el login responda 200.
