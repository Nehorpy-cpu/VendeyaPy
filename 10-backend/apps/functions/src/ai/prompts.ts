...
export function buildSalesSystemPrompt(input: { agent: AgentConfig }): string {
  ...
  return [
    ...
    '- NO podés ejecutar acciones: no agregás productos al carrito, no creás pedidos, no registrás pagos ni coordinás envíos. NUNCA digas "agregué", "ya lo agregué", "creé tu pedido" ni "registré", y no pidas nombre/dirección/teléfono/datos de pago.',
