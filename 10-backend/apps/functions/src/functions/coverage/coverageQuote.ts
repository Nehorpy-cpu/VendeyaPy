  ...
  return db().runTransaction(async (tx): Promise<TxAOutcome> => {
    ...
    if (req.status !== 'pending_coverage_review') {
      ...
      if (
        ...
        return { kind: 'ok_idempotente', shippingGs: req.shippingQuote.chargeGs, totalGs: subtotal + req.shippingQuote.chargeGs };
      }
    ...
    // ---- RECUPERACIÓN: pointer + outbox ANTES de la expiración y de los gates de huellas
    // (diseño H2 + review 3C): un envío que PUDO llegar al cliente (sent/sending/unknown) JAMÁS
    // se abandona por vencimiento sin estado terminal auditable — el vencimiento de esos caminos
    // lo aplica TX-C (sent_not_applied) o la resolución manual, nunca un expire ciego.
    ...
    const outbox: CoverageOutboxMessage = {
...
async function enviarQuote(tenantId: string, requestId: string, attemptId: string): Promise<void> {
  ...
  // 1) Transporte estricto: mismo receivedVia, credenciales del PROPIO tenant, live.
  const client = await getWhatsAppClientExact(tenantId, obPre.receivedVia ?? null);
...
async function txAplicarAprobacion(tenantId: string, requestId: string, attemptId: string): Promise<{ shippingGs: number; totalGs: number; customerId: string }> {
  ...
  try {
    out = await db().runTransaction(async (tx) => {
      ...
      // Idempotencia: TX-C ya commiteó para ESTE intento.
      if (req.status === 'coverage_approved' && req.shippingQuote?.sourceOutboxId === obId) {
        return { shippingGs: req.shippingQuote.chargeGs, totalGs: (req.cartSnapshot?.subtotal ?? 0) + req.shippingQuote.chargeGs, customerId: req.customerId };
      }
      ...
      return { shippingGs: pending.chargeGs, totalGs: cart.subtotal + pending.chargeGs, customerId: req.customerId };
    });
  } catch (e) {
