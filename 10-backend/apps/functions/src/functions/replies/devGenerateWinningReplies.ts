/**
 * devGenerateWinningReplies — Mina respuestas ganadoras (P18, dev/job)
 * ===================================================================
...
export const devGenerateWinningReplies = onRequest({ region: 'us-central1', cors: true }, async (req, res) => {
  ...
  try {
    ...
    for (const t of tenants) out[t] = await generateWinningReplies(t);
    res.json({ ok: true, replies: out });
  } catch (e) {
    logger.error('Error en devGenerateWinningReplies', e);
