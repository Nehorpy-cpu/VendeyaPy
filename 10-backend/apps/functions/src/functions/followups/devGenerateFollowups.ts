/**
 * devGenerateFollowups — Genera tareas de seguimiento (P14, dev/job)
 * ==================================================================
...
export const devGenerateFollowups = onRequest({ region: 'us-central1', cors: true }, async (req, res) => {
  ...
  try {
    ...
    for (const t of tenants) out[t] = await generateFollowUpTasks(t);
    res.json({ ok: true, followups: out });
  } catch (e) {
    logger.error('Error en devGenerateFollowups', e);
