...
import type { Order, OrderStatus, Product } from '@vpw/shared';
...
export function computeMetrics(orders: Order[], products: Product[]): DashboardMetrics {
  const vendidos = orders.filter((o) => isPaidStatus(o.status));
  const ingresos = vendidos.reduce((s, o) => s + o.totals.total, 0);

  const conCosto = vendidos.filter((o) => o.totals.grossProfit != null);
  const costoIncompleto = vendidos.some((o) => o.totals.grossProfit == null);
  const ganancia = conCosto.length ? conCosto.reduce((s, o) => s + (o.totals.grossProfit ?? 0), 0) : null;
  const costos = conCosto.length ? conCosto.reduce((s, o) => s + (o.totals.totalCost ?? 0), 0) : null;
  ...
  for (const o of vendidos) {
    for (const it of o.items) {
      ...
      agg.profit = agg.profit == null || it.grossProfit == null ? null : agg.profit + it.grossProfit;
